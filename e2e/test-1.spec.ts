...
test('test', async ({ page }) => {
  ...
  await page.locator('div').filter({ hasText: /^Ägg 15p Frigående Inomhus Medium39,90 kr$/ }).getByRole('heading').click();
  await page.getByRole('link', { name: 'Vegetariskt' }).nth(1).click();
  await page.getByRole('link', { name: 'Fryst vegetariskt' }).click();
  await page.locator('div').filter({ hasText: /^Vegetariska Delikatessbullar Fryst48,90 kr$/ }).getByRole('img').click();
  await page.getByRole('link', { name: 'Dryck' }).click();
  await page.getByRole('link', { name: 'Öl & cider' }).click();
  await page.locator('div').filter({ hasText: /^Melleruds Pilsner Eko 0,5% Alkfri Glas12,50 kr$/ }).getByRole('img').click({
    button: 'right'
  });
  await page.locator('div').filter({ hasText: /^Melleruds Pilsner Eko 0,5% Alkfri Glas12,50 kr$/ }).getByRole('img').click();
  await page.getByRole('link', { name: 'Saft & stilldrinks' }).dblclick();
  await page.getByRole('link', { name: 'Stilldrinks', exact: true }).click();
  await page.locator('div').filter({ hasText: /^Pärondryck Mindre Socker Fruktdryck Tetra11,90 kr$/ }).getByRole('img').click();
  ...
  await page.locator('div').filter({ hasText: /^Velo Crispy Peppermint Tobaksfritt Snus41,90 kr$/ }).getByRole('heading').click();
